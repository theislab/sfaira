...
class Dataset(DatasetBase):
    ...
    def __init__(
        ...
        self.species = "mouse"
        self.id = "mouse_fat_2019_smartseq2_pisco_003_10.1101/661728"
        self.source = source
        if self.source == "aws":
    ...
    def _load(self, fn=None):
        if fn is None:
            ...
            if self.source == "aws":
                fn = os.path.join(self.path, "mouse/fat/tabula-muris-senis-facs-processed-official-annotations-MAT.h5ad")
            elif self.source == "figshare":
                fn = os.path.join(self.path, "mouse/fat/mat_facs.h5ad")
            else:
                raise ValueError("source %s not recognized" % self.source)
