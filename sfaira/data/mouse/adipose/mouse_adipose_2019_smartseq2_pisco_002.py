...
class Dataset(DatasetBase):
    ...
    def _load(self, fn=None):
        if fn is None:
            ...
            if self.source == "aws":
                fn = os.path.join(self.path, "mouse", "adipose", "tabula-muris-senis-facs-processed-official-annotations-GAT.h5ad")
            elif self.source == "figshare":
                fn = os.path.join(self.path, "mouse", "adipose", "gat_facs.h5ad")
            else:
                raise ValueError("source %s not recognized" % self.source)
