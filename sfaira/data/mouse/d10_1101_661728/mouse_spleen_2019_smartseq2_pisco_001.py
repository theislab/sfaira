...
class Dataset(DatasetTms):
    ...
    def __init__(
        ...
        super().__init__(self=self, path=path, meta_path=meta_path, **kwargs)        self.id = "mouse_spleen_2019_smartseq2_pisco_001_10.1101/661728"
        self.source = source
        if self.source == "aws":
    ...
    def _load(self, fn=None):
        if fn is None:
            ...
            if self.source == "aws":
                fn = os.path.join(self.path, "mouse", "spleen", "tabula-muris-senis-facs-processed-official-annotations-Spleen.h5ad")
            elif self.source == "figshare":
                fn = os.path.join(self.path, "mouse", "spleen", "Spleen_facs.h5ad")
            else:
                raise ValueError("source %s not recognized" % self.source)
