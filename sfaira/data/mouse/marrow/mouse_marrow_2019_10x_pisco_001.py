...
class Dataset(DatasetBase):
    ...
    def __init__(
        ...
        self.species = "mouse"
        self.id = "mouse_marrow_2019_10x_pisco_001_10.1101/661728"
        self.source = source
        if self.source == "aws":
    ...
    def _load(self, fn=None):
        if fn is None:
            ...
            if self.source == "aws":
                fn = os.path.join(self.path, "mouse/marrow/tabula-muris-senis-droplet-processed-official-annotations-Marrow.h5ad")
            elif self.source == "figshare":
                fn = os.path.join(self.path, "mouse/marrow/Marrow_droplet.h5ad")
            else:
                raise ValueError("source %s not recognized" % self.source)
