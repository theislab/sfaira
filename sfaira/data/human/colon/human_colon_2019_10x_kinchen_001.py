...
class Dataset(DatasetBase):
    ...
    def _load(self, fn=None):
        ...
        self.adata.uns["doi"] = "10.1016/j.cell.2018.08.067"
        ...
        self.adata.obs["healthy"] = [line == 'normal' for line in
                                     self.adata.obs['donor_organism.diseases.ontology_label']]
        self.adata.obs["state_exact"] = self.adata.obs['donor_organism.diseases.ontology_label'].astype('category')
        self.adata.obs["state_exact"] = self.adata.obs["state_exact"]\
            .cat.rename_categories({'normal': 'healthy', 'ulcerative colitis (disease)': 'ulcerative colitis'})

        self._convert_and_set_var_names(symbol_col='names', ensembl_col='Accession', new_index='ensembl')
