...
dir_data = ""
dir_meta = os.path.join(os.path.dirname(os.path.dirname(__file__)), "meta")
dir_cache = os.path.join(os.path.dirname(os.path.dirname(__file__)), "cache")
        ...
        for d in os.listdir(cwd):
            if os.path.isdir(os.path.join(cwd, d)):  # Iterate over mock studies (directories).
                if d[:len(dir_prefix)] == dir_prefix and d not in dir_exclude:  # Narrow down to data set directories
                    path_base = f"sfaira.unit_tests.mock_data.loaders.{d}"
