...
@pytest.mark.parametrize("match_to_reference", [{"homosapiens": ASSEMBLY_HUMAN, "musmusculus": ASSEMBLY_MOUSE}, ])
...
def test_streamline_features(database: str, subset_args: List[str], match_to_reference: dict,
    ...
    dsg.load()
    dsg.streamline_features(match_to_release=match_to_reference, subset_genes_to_type=subset_genes_to_type)
...
def test_streamline_metadata(database: str, subset_args: List[str], format: str):
    ...
    dsg.subset(key=subset_args[0], values=subset_args[1])
    dsg.load()
    dsg.streamline_features(match_to_release={"homosapiens": ASSEMBLY_HUMAN, "musmusculus": ASSEMBLY_MOUSE},
