...
def test_dsgs_subset_dataset_wise(organ: str):
    ...
    for x in ds.dataset_groups:
        for k, v in x.datasets.items():
            assert v.organism == "human", v.organism
...
@pytest.mark.parametrize("match_to_reference", [ASSEMBLY_HUMAN, {"human": ASSEMBLY_HUMAN}])
...
def test_dsgs_streamline_features(match_to_reference: str, remove_gene_version: bool, subset_genes_to_type: str):
    ...
    gc = ds.get_gc(match_to_reference["human"] if isinstance(match_to_reference, dict) else match_to_reference)
